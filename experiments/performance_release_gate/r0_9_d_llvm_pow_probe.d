// Benchmark-only LDC probe for color-d #5.
//
// Keeps the original R0.9 public-function attributes and CTFE behavior.
// At compile time, runtimeFastPow() delegates to std.math.pow.
// At runtime under LDC, it delegates to llvm_pow(T,T).
//
// This is not production policy. It exists to verify whether the LDC
// intrinsic can preserve @safe pure nothrow @nogc + CTFE while eliminating
// the x86-64 Phobos real/x87 pow bottleneck.
...
T runtimeFastPow(T)(T base, T exponent)
@safe pure nothrow @nogc
if (isColorScalar!T)
{
    if (__ctfe)
        return cast(T)pow(base, exponent);

    return llvm_pow!T(base, exponent);
}
...
@safe pure nothrow @nogc
{
    ...
    return
        signOf(encoded) *
        runtimeFastPow!T(base, cast(T)2.4);
}
...
{
    writeln(
        "=== color-d R0.9 LDC llvm.pow runtime probe ==="
    );
